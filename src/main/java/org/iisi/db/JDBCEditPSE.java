...
public class JDBCEditPSE extends JDBCCore {
	...
	public int ePSE(int Pid, String ad) {
		...
		try {
			...
			PreparedStatement st = conn.prepareStatement(
					"update PSE.PSE_MAIN set STATUS=3,APPLYTIME = to_date(?,'YYYY/MM/DD HH24/MI') where PID=?");
			st.setString(1, ad);
			st.setInt(2, Pid);
			st.executeUpdate();
	...
	public int eSubPSE(int Pid, int Pcid, String pk, String ds, String de, String ts, String te, String total,
			String ps) {
		int r = 0;
		...
		try {
			...
			PreparedStatement st = conn.prepareStatement(
					"update PSE.PSE_SUB set STARTDATETIME = to_date(?,'YYYY-MM-DDHH24:MI'),ENDDATETIME = to_date(?,'YYYY-MM-DDHH24:MI'),PCTOTAL = ?, KID=?, PS=?  where PID=? and PCID=?");
	...
	public List<Holiday> getHolidays() {
		...
		try {
			...
			Connection conn;
			conn = makeConnection();
			PreparedStatement pstmt = conn.prepareStatement("select to_char(H_Date,'yyyy-mm-dd'),H_NAME from holiday ");
