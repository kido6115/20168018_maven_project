...
import org.iisi.db.JDBCPSESearch.FileStatus;
...
public class JDBCSearchHour extends JDBCCore {
	...
	public List<SearchHourMng> SearchHour_m(String year, String eid,
	...
	dep=null;
		...
		try {
			...
			while (rs.next()) {
				...
				SearchHourMng dl = new SearchHourMng(seid, sname, syear, skname,
						scredit);
				sh.add(i, dl);
				i++;
			}
	...
	public List<SearchHourEmp> SearchHour_e(String Year, String eid, String dep) {
		...
		try {
			...
			while (rs.next()) {
				...
				SearchHourEmp dl = new SearchHourEmp(seid, sname, syear, skname,
						scredit);
				sh.add(i, dl);
				i++;
			}
