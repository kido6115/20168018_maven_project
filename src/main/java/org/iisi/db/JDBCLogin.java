...
public class JDBCLogin extends JDBCCore {
	...
	public int getJobId(String Account) {
		...
		try {
			Connection conn;
			conn = makeConnection();
			PreparedStatement st = conn.prepareStatement("Select JOB_ID from EMPLOYEE where EID='" + Account + "'");
			// st.setString(1, Account);
			ResultSet rs = st.executeQuery();
			while (rs.next()) {
	...
	public String getName(String Account) {
		...
		try {
			Connection conn;
			conn = makeConnection();
			PreparedStatement st = conn.prepareStatement("Select Name from EMPLOYEE where EID='" + Account + "'");
			// st.setString(1, Account);
			ResultSet rs = st.executeQuery();
			while (rs.next()) {
	...
	public String getMail(String Account) {
		...
		try {
			Connection conn;
			conn = makeConnection();
			PreparedStatement st = conn.prepareStatement("Select Email from EMPLOYEE where EID='" + Account + "'");
			ResultSet rs = st.executeQuery();
			while (rs.next()) {
	...
	public int getStatus(String Account) {
		...
		try {
			Connection conn;
			conn = makeConnection();
			PreparedStatement st = conn.prepareStatement("Select DEP_ID from EMPLOYEE where EID='" + Account + "'");
			ResultSet rs = st.executeQuery();
			while (rs.next()) {
	...
	public String getJob(String Account) {
		...
		try {
			...
			PreparedStatement st = conn.prepareStatement(
					"Select a.JOB_NAME from JOB a,employee b where b.EID='" + Account + "'and a.job_id=b.job_id ");
			// st.setString(1, Account);
			ResultSet rs = st.executeQuery();
			while (rs.next()) {
	...
	public String getDep(String Account) {
		...
		try {
			...
			conn = makeConnection();
			PreparedStatement st = conn.prepareStatement(
					"Select a.DEP_NAME from DEP a,employee b where b.EID='" + Account + "'and a.dep_id=b.dep_id ");
			// st.setString(1, Account);
			ResultSet rs = st.executeQuery();
			while (rs.next()) {
